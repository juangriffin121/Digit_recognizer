...
def predict(red, correctos, totales):
    ...
    print(
        "Dibuje en paint en una imagen de 200 x 200 pixeles con blanco sobre negro el digito que quiere que Dajij dWkikikant prediga"
    )
    image = get_image()
    ...
    correctos += is_right
    print(
        f"wkiki viene acertando {correctos}/{totales} osea un {correctos/totales*100}%"
    )
    save_data = boolean(
        input("Quiere guardar este dato para que wwkiki aprendda?(s/n)"),
        "Quiere guardar este dato para que wwkiki aprendda?(s/n)",
    )
    if save_data:
        add_data(is_right, main_prediction, image)

    graph = boolean(
        input("quiere graficar lo que ve Wkiki?"), "quiere graficar lo que ve Wkiki?"
    )
    if graph:
        input_ = image
        print(red)
...
def get_image():
    image_path = input(
        "Guarde el archivo como .bmp 256 color y sueltelo sobre la consola"
    )
    clear()
...
def handle_feedback(prediction_vector):
    ...
    if wkiki_is_right:
        ...
        print(
            "si sirve de algo estas son las probabilidades que le daba wkiki a cada digito"
        )
...
def main():
    try:
        ...
        print(e.message)
        again = boolean(
            input("Algo salio mal quiere intentar de nuevo?(s/n)"),
            "Algo salio mal quiere intentar de nuevo?(s/n)",
        )
        if again:
            main()
        else:
